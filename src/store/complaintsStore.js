...
const SEED_ANNOUNCEMENTS = [
  {
    ...
    content: 'Major breakthrough in Chamrajapet Ward road connectivity! The Main Market Road reconstruction is 75% complete and ahead of schedule. Thank you for your patience during the construction phase.',
  ...
  },
  {
    ...
    category: 'Meeting',
    content: 'Reminder: The monthly Ward Grievance Redressal Meeting will be held this Sunday at the Community Center from 10 AM to 2 PM. All residents are invited to share their feedback.',
  ...
  },
  {
    ...
    category: 'Health',
    content: 'Free Health & Eye Checkup Camp organized at Government Primary School this Saturday. Specialized doctors from Victoria & Minto Hospital will be available from 9 AM to 4 PM.',
...
const SEED_ALERTS = [
  { id: 'alert-1', text: '🚨 ROAD CLOSURE: Main Street near Jayanagar 4th Block closed for repairs until June 15. Use alternate route via Gandhi Nagar.', active: true, createdAt: new Date().toISOString() },
  { id: 'alert-2', text: '⚠️ WATER SUPPLY: Scheduled maintenance on June 14 from 8AM–2PM. Please store water in advance.', active: true, createdAt: new Date().toISOString() },
  { id: 'alert-3', text: '📢 PUBLIC MEETING: Ward Development Committee meeting on June 15 at 6PM — Town Hall, Room 3. All residents invited.', active: true, createdAt: new Date().toISOString() },
  ...
  persist(
    (set, get) => ({
      ...
      addComplaint: async (data) => {
        const state = get()
        const wardComplaints = state.complaints.filter(c => c.ward === data.ward).length
        ...
        try {
          ...
          if (res.ok) {
            ...
            set(s => {
              const citizen = s.citizens[data.voterId] || { name: data.fullName, karma: 0, badges: [] }
              return {
                complaints: [savedComplaint, ...s.complaints],
                citizens: {
                  ...s.citizens,
                  [data.voterId]: { ...citizen, name: data.fullName, karma: citizen.karma + 50 }
                }
              }
            })
          }
      ...
      upvoteComplaint: (complaintId, voterId, fullName) => {
        set(s => {
          ...
          const updatedComplaints = s.complaints.map(c => 
            c.id === complaintId ? { ...c, upvotes: [...(c.upvotes || []), voterId] } : c
          )
          
          const citizen = s.citizens[voterId] || { name: fullName, karma: 0, badges: [] }
          
          return {
            complaints: updatedComplaints,
            citizens: {
              ...s.citizens,
              [voterId]: { ...citizen, name: fullName || citizen.name, karma: citizen.karma + 10 }
            }
          }
        })
      },
        ...
        try {
          ...
          if (res.ok) {
            ...
            set(s => {
              ...
              let updatedComplaints = s.complaints.map(c => c.id === id ? updatedComplaint : c)
              ...
              if (status === 'Resolved' && afterPhotoData) {
                 ...
                 if (comp && comp.photoData) {
                   showcase = [
                     { id: comp.id, ward: comp.ward, category: comp.category, title: comp.description, before: comp.photoData, after: afterPhotoData, date: new Date().toISOString() },
                     ...s.beforeAfter
                   ]
                 }
              }

              return { complaints: updatedComplaints, beforeAfter: showcase, citizens: updatedCitizens }
            })
          }
        } catch (e) {
      ...
      beforeAfter: [
        {
          ...
          story: 'Over 500 citizens received free medical checkups today. Heartfelt thanks to our dedicated volunteer doctors and nurses from Fortis Hospital for making this possible! Together, we ensure every citizen has access to basic healthcare.',
          images: [
        ...
        },
        {
          ...
          date: new Date().toISOString(),
          before: 'https://res.cloudinary.com/dvf40xmxp/image/upload/v1703666249/pothole_before_qpwweq.jpg',
          after: 'https://res.cloudinary.com/dvf40xmxp/image/upload/v1703666250/pothole_after_jrt2xw.jpg'
        },
        {
          id: 'mock-3',
          type: 'standalone',
          ...
          story: 'Our phenomenal youth volunteers spent their Sunday morning clearing out plastic waste from the local lake. The transformation is incredible. Let\'s keep our water bodies clean and green!',
          images: [
      ...
      addStandaloneActivity: (data) => {
        set(s => ({
          beforeAfter: [
            { id: Date.now().toString(), type: 'standalone', date: new Date().toISOString(), ...data },
            ...s.beforeAfter
          ]
        }))
      },
      ...
      addAlert: (text) => {
        const alert = { id: `alert-${Date.now()}`, text, active: true, createdAt: new Date().toISOString() }
        set(s => ({ liveAlerts: [alert, ...s.liveAlerts] }))
        return alert.id
      },

      updateAlert: (id, text) => {
        set(s => ({ liveAlerts: s.liveAlerts.map(a => a.id === id ? { ...a, text } : a) }))
      },
      ...
      toggleAlert: (id) => {
        set(s => ({ liveAlerts: s.liveAlerts.map(a => a.id === id ? { ...a, active: !a.active } : a) }))
      },
      ...
      likeAnnouncement: async (id) => {
        try {
          ...
          if (res.ok) {
            ...
            set(s => ({
              announcements: s.announcements.map(a => a.id === id ? updatedAnn : a)
            }));
          }
        } catch (e) {
      ...
      addVolunteer: async (vol) => {
        try {
          ...
          if (res.ok) {
            ...
            set(s => ({ volunteers: [newVolunteer, ...(s.volunteers || [])] }));

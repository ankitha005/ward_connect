...
import {
  Users, MessageSquare, CheckCircle, Clock, LogOut,
  Search, Activity, ShieldCheck, Megaphone, Map,
  Phone, MapPin, User, FileText, ChevronDown, ChevronUp,
  X, ArrowLeft, AlertCircle, XCircle, BarChart3, AlertTriangle, Plus, Pencil, Save, Eye, EyeOff, Trash2, Sparkles
} from 'lucide-react'
...
function HeatMap({ complaints }) {
  ...
  useEffect(() => {
    ...
    const points = complaints.map(c => {
      ...
      const intensity = c.priority === 'Urgent' ? 1.0 : c.priority === 'High' ? 0.7 : 0.4
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
      <div className="p-5 border-b border-slate-50 flex items-center justify-between">
        <div>
          <h3 className="font-bold text-dark flex items-center gap-2"><Map size={18} className="text-primary" /> Complaint Heat Map</h3>
          <p className="text-xs text-slate-400 font-medium mt-0.5">Brighter zones = higher complaint density · {complaints.length} total points</p>
        </div>
        <div className="flex items-center gap-3 text-[10px] font-bold uppercase tracking-wider">
          ...
          <span className="flex items-center gap-1"><span className="w-3 h-3 rounded-full bg-amber-400 inline-block" />Med</span>
    ...
    <div className="fixed inset-0 z-50 flex">
      ...
      >
        ...
        <div className="sticky top-0 bg-white border-b border-slate-100 px-6 py-4 flex items-center justify-between z-10">
          <div>
            <div className="font-mono font-bold text-primary text-base">{complaint.id}</div>
        ...
        <div className="px-6 py-6 space-y-8">
          {/* Citizen Info */}
          <section>
            <h4 className="label-style flex items-center gap-1.5 mb-4"><User size={12} /> Citizen Details</h4>
            <div className="bg-slate-50 rounded-2xl p-5 space-y-3">
              <Row label="Full Name" value={complaint.fullName} />
              <Row label="Mobile" value={
                <a href={`tel:+91${complaint.mobile}`} className="text-primary font-bold hover:underline flex items-center gap-1.5">
                  <Phone size={13} /> +91 {complaint.mobile}
                </a>
              } />
              <Row label="Voter ID" value={<span className="font-mono">{complaint.voterId}</span>} />
            </div>
          </section>

          {/* Location Info */}
          <section>
            <h4 className="label-style flex items-center gap-1.5 mb-4"><MapPin size={12} /> Location</h4>
            <div className="bg-slate-50 rounded-2xl p-5 space-y-3">
              <Row label="Ward" value={WARD_LABELS[complaint.ward]} />
              <Row label="Address" value={complaint.address} />
              {complaint.area && <Row label="Area / Colony" value={complaint.area} />}
              {complaint.lat && complaint.lng && (
                <Row label="GPS Coordinates" value={
                  <a
          ...
          {/* Issue Details */}
          <section>
            <h4 className="label-style flex items-center gap-1.5 mb-4"><FileText size={12} /> Issue Details</h4>
            <div className="bg-slate-50 rounded-2xl p-5 space-y-3">
              ...
              <div>
                <div className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1.5">Description</div>
                <p className="text-slate-600 text-sm leading-relaxed font-medium">{complaint.description}</p>
              </div>
              
              {complaint.photoData && (
                <div className="mt-4">
                   <div className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1.5">Attached Image (Before)</div>
                   <img src={complaint.photoData} alt="Issue" className="w-full h-48 object-cover rounded-xl border border-slate-200" />
                </div>
              )}
            </div>
          </section>
          ...
          {/* Status Timeline */}
          <section>
            <h4 className="label-style flex items-center gap-1.5 mb-4"><Activity size={12} /> Status Timeline</h4>
            <div className="space-y-3">
              {complaint.statusHistory.map((h, i) => (
                <div key={i} className="flex gap-3">
                  <div className="flex flex-col items-center">
                    <div className={`w-3 h-3 rounded-full mt-1 ${i === complaint.statusHistory.length - 1 ? 'bg-primary' : 'bg-slate-300'}`} />
                    {i < complaint.statusHistory.length - 1 && <div className="w-0.5 h-6 bg-slate-200 mt-1" />}
                  </div>
                  <div>
                    <div className="text-sm font-bold text-dark">{h.status}</div>
                    {h.note && <div className="text-xs text-slate-500 font-medium">{h.note}</div>}
                    <div className="text-[10px] text-slate-400">{new Date(h.date).toLocaleString('en-IN')}</div>
                  </div>
                </div>
              ))}
            </div>
          ...
          <section className="bg-primary/5 border border-primary/10 rounded-2xl p-5 space-y-4">
            <h4 className="font-bold text-dark text-sm">Update Complaint Status</h4>
            <div>
              <label className="label-style">New Status</label>
              <select value={newStatus} onChange={e => setNewStatus(e.target.value)} className="input-style">
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                ...
                >
                  <Sparkles size={12} /> {generatingAi ? 'Generating...' : '✨ AI Generate Note'}
                </button>
              </div>
              <textarea value={note} onChange={e => setNote(e.target.value)} className="input-style resize-none" rows={3}
            ...
            {newStatus === 'Resolved' && complaint.photoData && (
              <div className="bg-green-50/50 p-4 rounded-xl border border-green-100">
                <label className="label-style text-green-700">Upload "After" Image (Social Activity Showcase)</label>
                <input 
                  type="file" 
                  accept="image/*" 
                ...
                />
                <p className="text-[10px] text-slate-500 mt-2 font-medium">Providing an After image will automatically publish this resolution as a "Before & After" slide on the Social Activity page.</p>
                {afterImage && (
                  <img src={afterImage} alt="After" className="mt-3 w-full h-32 object-cover rounded-xl shadow-sm border border-green-200" />
                )}
              </div>
            )}
...
function AlertsTab() {
  const { liveAlerts, addAlert, updateAlert, deleteAlert, toggleAlert } = useComplaintsStore()
  ...
  const startEdit = (a) => { setEditId(a.id); setEditText(a.text) }
  const saveEdit = () => { updateAlert(editId, editText.trim()); setEditId(null) }

  return (
    <div className="space-y-6">
      {/* Add Alert */}
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
        <h3 className="font-bold text-dark flex items-center gap-2 mb-4"><AlertTriangle size={16} className="text-red-500" /> Add New Live Alert</h3>
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        <div className="p-5 border-b border-slate-50 flex items-center justify-between">
          <h3 className="font-bold text-dark">{liveAlerts.length} Live Alert{liveAlerts.length !== 1 ? 's' : ''}</h3>
          <span className="text-xs text-slate-400 font-medium">{liveAlerts.filter(a => a.active).length} currently visible on site</span>
        </div>

        {liveAlerts.length === 0 ? (
          <div className="py-12 text-center text-slate-400 font-semibold">No alerts. Add one above!</div>
        ) : (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    {editId === alert.id ? (
                      ...
                      <p className="text-sm font-semibold text-slate-700 leading-snug">{alert.text}</p>
                    )}
                    <p className="text-[10px] text-slate-400 mt-1 font-medium">
                      Added {new Date(alert.createdAt).toLocaleString('en-IN')}
                      <span className={`ml-3 font-bold ${alert.active ? 'text-green-600' : 'text-slate-400'}`}>
                  ...
                  <div className="flex items-center gap-1 shrink-0">
                    ...
                    <button onClick={() => toggleAlert(alert.id)}
                      title={alert.active ? 'Hide from ticker' : 'Show in ticker'}
...
export default function AdminDashboard() {
  const { complaints, updateStatus, isAdminLoggedIn, adminLogout, volunteers, surveys } = useComplaintsStore()
  ...
  const exportCSV = () => {
    ...
    const headers = ['Complaint ID', 'Date', 'Full Name', 'Mobile', 'Voter ID', 'Ward', 'Category', 'Priority', 'Status', 'Description']
    const rows = filtered.map(c => [
      c.id, new Date(c.createdAt).toLocaleDateString('en-IN'), c.fullName, c.mobile, c.voterId, WARD_LABELS[c.ward] || c.ward,
      c.category, c.priority, c.status, `"${c.description.replace(/"/g, '""')}"`
    ])
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col">
      ...
      <header className="bg-white border-b border-slate-100 px-6 py-4 flex items-center justify-between sticky top-0 z-40 shadow-sm">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 bg-gradient-to-br from-primary to-orange-400 rounded-xl flex items-center justify-center text-white font-bold text-sm shadow-md shadow-primary/30">A</div>
          <div>
            <div className="font-bold text-dark text-sm">Ward Admin Panel</div>
            <div className="text-[10px] text-slate-400 font-medium uppercase tracking-wider">ADDA_360 Ward Connect</div>
          </div>
        </div>
      ...
      <div className="max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-8 space-y-6">
        ...
        {tab === 'overview' && (
          <>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-5">
              {[
                { label: 'Total Complaints', value: total,      icon: <MessageSquare />, color: 'text-primary bg-primary/5',    ring: 'ring-primary/20' },
                { label: 'Pending',          value: pending,    icon: <Clock />,         color: 'text-amber-500 bg-amber-50',   ring: 'ring-amber-200' },
                { label: 'In Progress',      value: inProgress, icon: <Activity />,      color: 'text-blue-500 bg-blue-50',     ring: 'ring-blue-200' },
                { label: 'Resolved',         value: resolved,   icon: <CheckCircle />,   color: 'text-success bg-success/5',    ring: 'ring-green-200' },
              ].map((stat, i) => (
            ...
            <div className="bg-gradient-to-r from-red-600 via-red-500 to-amber-500 rounded-2xl p-6 text-white shadow-lg flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
              <div className="space-y-1">
                <div className="inline-flex items-center gap-1.5 bg-white/20 px-3 py-1 rounded-full text-xs font-extrabold uppercase tracking-widest backdrop-blur-md">
                  <Sparkles size={14} className="text-amber-200" /> AI Ward Intelligence
                </div>
                <h3 className="text-xl font-black">Real-Time Hotspot & Triage Analysis</h3>
                <p className="text-xs text-red-100 font-medium max-w-xl">
                  AI has detected <span className="font-bold underline text-white">2 cluster zones</span> requiring immediate municipal attention in Bengaluru Wards (Road Repairs & Drainage). Auto-GPS routing active.
                </p>
              </div>
              <button
            ...
            {topCats.length > 0 && (
              <div className="bg-white rounded-2xl border border-slate-100 p-6 shadow-sm">
                <h3 className="font-bold text-dark mb-4 flex items-center gap-2"><BarChart3 size={18} className="text-primary" /> Top Issue Categories</h3>
                <div className="h-52">
                  <Bar data={{
                    labels: topCats.map(([k]) => k),
                    datasets: [{ data: topCats.map(([,v]) => v), backgroundColor: '#DC2626', borderRadius: 8, hoverBackgroundColor: '#b91c1c' }]
                  }} options={chartOpts} />
                </div>
              </div>
            )}
            ...
            <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
              ...
              {complaints.slice(0, 5).length === 0 ? (
                <p className="text-slate-400 text-sm font-medium">No complaints yet.</p>
              ) : (
                complaints.slice(0, 5).map(c => (
                  <div key={c.id} onClick={() => { setSelected(c); setTab('complaints') }}
                    ...
                    <div>
                      <div className="font-mono font-bold text-primary text-xs">{c.id}</div>
                      <div className="text-sm font-semibold text-dark">{c.fullName} · {c.category}</div>
                    </div>
                    <div className="flex items-center gap-2">
                      <span className={`text-[10px] font-bold px-2.5 py-1 rounded-full border ${STATUS_COLORS[c.status] || ''}`}>{c.status}</span>
                    </div>
                  </div>
                ))
              )}
          ...
          <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
            ...
            <div className="p-5 border-b border-slate-50 flex flex-wrap gap-4 items-center justify-between">
              <div className="flex flex-wrap gap-3 items-center flex-1">
                ...
                <div className="flex items-center gap-2 border border-slate-200 rounded-xl px-3 bg-slate-50">
                  <input type="date" value={dateStart} onChange={e => setDateStart(e.target.value)} className="text-sm py-2 bg-transparent outline-none font-semibold text-slate-500" />
                  <span className="text-slate-300">-</span>
                  <input type="date" value={dateEnd} onChange={e => setDateEnd(e.target.value)} className="text-sm py-2 bg-transparent outline-none font-semibold text-slate-500" />
                </div>
              </div>
            ...
            {filtered.length === 0 ? (
              <div className="py-16 text-center text-slate-400 font-semibold">
                {complaints.length === 0 ? 'No complaints filed yet.' : 'No results match your filters.'}
              </div>
            ) : (
                  ...
                  <tbody className="divide-y divide-slate-50">
                    {filtered.map(c => (
                      <tr key={c.id} className="hover:bg-slate-50 transition-colors">
                        <td className="px-5 py-3 font-mono font-bold text-xs text-primary">{c.id}</td>
                        <td className="px-5 py-3 font-semibold text-dark">{c.fullName}</td>
                        <td className="px-5 py-3">
                          <a href={`tel:+91${c.mobile}`} className="flex items-center gap-1 text-slate-600 hover:text-primary font-medium transition-colors">
                            <Phone size={12} /> {c.mobile}
                          </a>
                        </td>
                        <td className="px-5 py-3 text-slate-500">{WARD_LABELS[c.ward]}</td>
                        ...
                        <td className="px-5 py-3">
                          <span className={`text-[10px] font-bold px-2.5 py-1 rounded-full border ${STATUS_COLORS[c.status] || ''}`}>
                            {c.status}
                          </span>
                        </td>
                        <td className="px-5 py-3">
                          {c.lat
                            ? <a href={`https://www.google.com/maps?q=${c.lat},${c.lng}`} target="_blank" rel="noreferrer" className="text-primary text-xs hover:underline font-bold flex items-center gap-1"><MapPin size={11} />View</a>
                            : <span className="text-slate-300 text-xs">—</span>
                          }
                        </td>
                        <td className="px-5 py-3 text-slate-400 text-xs">{new Date(c.createdAt).toLocaleDateString('en-IN')}</td>
        ...
        {tab === 'volunteers' && (
          <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 overflow-x-auto">
            <h3 className="font-bold text-dark mb-4 flex items-center gap-2"><Users size={18} className="text-primary"/> Registered Volunteers ({volunteers?.length || 0})</h3>
            {!volunteers || volunteers.length === 0 ? (
              <p className="py-8 text-center text-slate-400 font-semibold">No volunteers registered yet.</p>
            ) : (
              <table className="w-full text-sm">
                <thead>
                ...
                <tbody className="divide-y divide-slate-50">
                  {volunteers.map(v => (
                    <tr key={v.id} className="hover:bg-slate-50 transition-colors">
                      <td className="py-3 pr-4 font-mono text-xs font-bold text-primary">{v.id}</td>
                      <td className="py-3 pr-4 font-semibold text-dark">{v.fullName}</td>
                      <td className="py-3 pr-4 font-medium text-slate-600">{v.mobile}</td>
                      <td className="py-3 pr-4 text-slate-500">{v.ward}</td>
                      <td className="py-3 pr-4 text-slate-500">{v.age || '--'}</td>
                      <td className="py-3 pr-4">
                        <div className="flex flex-wrap gap-1">
                          {v.skills?.map(s => <span key={s} className="bg-orange-50 text-orange-600 text-[10px] font-bold px-2 py-0.5 rounded-md uppercase">{s}</span>)}
                        </div>
                      </td>
                      <td className="py-3 text-xs text-slate-400 font-medium">{new Date(v.date).toLocaleDateString('en-IN')}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        {tab === 'surveys' && (
          <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 overflow-x-auto">
            <h3 className="font-bold text-dark mb-4 flex items-center gap-2"><FileText size={18} className="text-brand-green"/> Ward Survey Responses ({surveys?.length || 0})</h3>
            {!surveys || surveys.length === 0 ? (
              <p className="py-8 text-center text-slate-400 font-semibold">No survey responses yet.</p>
            ) : (
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-slate-100 text-left text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                ...
                <tbody className="divide-y divide-slate-50">
                  {surveys.map(s => (
                    <tr key={s.id} className="hover:bg-slate-50 transition-colors border-l-[3px] border-transparent hover:border-brand-green">
                      <td className="py-4 pr-4 pl-3 font-semibold text-dark">{s.citizen.name}</td>
                      <td className="py-4 pr-4 font-medium text-slate-500">{s.citizen.ward}</td>
                      <td className="py-4 pr-4 font-bold text-slate-700">{s.responses.roadQuality || '--'}</td>
                      <td className="py-4 pr-4 font-bold text-slate-700">{s.responses.safety || '--'}</td>
                      <td className="py-4 pr-4">
                        <span className="bg-green-50 text-green-700 font-bold px-3 py-1 text-xs rounded-xl">{s.responses.priorityArea || '--'}</span>
                      </td>
                      <td className="py-4 text-xs text-slate-400 font-medium">{new Date(s.date).toLocaleString('en-IN')}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <ComplaintDrawer
            ...
            onStatusUpdate={(id, status, note, afterImage) => {
              ...
              setSelected(prev => ({
                ...prev,
                status,
                statusHistory: [...prev.statusHistory, { status, note: note || '', date: new Date().toISOString() }]
              }))
            }}
          />
        )}

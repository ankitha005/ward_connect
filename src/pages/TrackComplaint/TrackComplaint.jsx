  ...
  const handleSearch = () => {
    ...
    if (mode === "id") {
      const found = complaints.find(
        (c) => c.id.toLowerCase() === search.trim().toLowerCase(),
      );
      found ? setResult([found]) : setNotFound(true);
    } else {
      const found = complaints.filter(
        (c) =>
          c.voterId.toLowerCase() === voterId.trim().toLowerCase() &&
          c.ward === ward,
      );
          ...
          {mode === "id" ? (
            <div className="flex flex-col sm:flex-row gap-4 max-w-2xl mx-auto">
              <div className="relative flex-1">
                <Search
                  className="absolute left-5 top-1/2 -translate-y-1/2 text-brand-orange"
                  size={20}
                />
                <input
                  value={search}
              ...
              >
                ...
                >
                  ...
                  <div className="flex items-center justify-between md:justify-end gap-5">
                    <span
                      className={`flex items-center gap-2 px-5 py-2.5 rounded-full text-sm font-bold border ${statusCfg.color}`}
                    >
                  ...
                  {isOpen && (
                    ...
                    >
                      <div className="border-t border-slate-100 bg-slate-50/50 p-6 md:p-8">
                        ...
                        <div>
                          <span className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-slate-400 mb-6">
                          ...
                          <div className="relative pl-6 space-y-8">
                            ...
                            {c.statusHistory.map((historyItem, i) => {
                              ...
                              const hCfg =
                                STATUS_CONFIG[historyItem.status] ||
                              ...
                              return (
                                ...
                                >
                                  {/* Dot */}
                                  <div
                                    className={`w-5 h-5 mt-0.5 shrink-0 rounded-full border-4 border-white shadow-sm flex items-center justify-center ${isLast ? hCfg.dot : "bg-slate-300"}`}
                                  >
                                    {isLast && (
                                  ...
                                  {/* Content */}
                                  <div
                                    className={`bg-white p-4 rounded-2xl border ${isLast ? hCfg.color.split(" ")[2] : "border-slate-100"} shadow-sm flex-1`}
                                  >
                                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1 mb-2">
                                      <h4
                                        className={`font-black uppercase tracking-wide text-sm ${isLast ? hCfg.color.split(" ")[1] : "text-slate-600"}`}
                                      >
                                        {historyItem.status}
                                      </h4>
                                    ...
                                    {historyItem.note ? (
                                      ...
                                      <p className="text-xs font-semibold text-slate-400 italic">
                                        No additional notes provided by
                                        administration.
                                      </p>
                                    )}
                                  </div>

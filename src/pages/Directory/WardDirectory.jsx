...
const WardDirectory = () => {
  ...
  const renderedGroups = GROUPS.filter(
    (g) => activeGroup === "All" || activeGroup === g,
  );

  // Filter logic for 33 Bangalore Wards
  const filteredWards = BANGALORE_WARDS_DATA.filter((w) => {
    const matchesSearch =
      w.constituency.toLowerCase().includes(mapSearch.toLowerCase()) ||
      w.corp.toLowerCase().includes(mapSearch.toLowerCase()) ||
      w.slNo.includes(mapSearch);
    const matchesCorp =
      activeCorp === "All Corporations" || w.corp === activeCorp;
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
          ...
          >
            <h1 className="text-4xl md:text-5xl font-extrabold text-dark tracking-tight mb-4">
              {mainTab === "maps"
                ? "🗺️ Bangalore Ward Maps & Directory"
          ...
          <div className="flex items-center justify-between mb-8 pb-4 border-b border-slate-200 flex-wrap gap-4">
            <div>
              ...
              <p className="text-xs text-slate-500 font-semibold mt-1">
                Showing {filteredWards.length} of 33 constituencies. Click "View
                Ward Maps" to launch live geographic boundary data.
              </p>
            </div>
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <span className="px-2 py-1 rounded bg-red-100 text-red-700 text-[10px] font-bold">
                Central
              </span>
              <span className="px-2 py-1 rounded bg-amber-100 text-amber-700 text-[10px] font-bold">
                North
              </span>
              <span className="px-2 py-1 rounded bg-blue-100 text-blue-700 text-[10px] font-bold">
                East
              </span>
              <span className="px-2 py-1 rounded bg-emerald-100 text-emerald-700 text-[10px] font-bold">
                South
              </span>
              <span className="px-2 py-1 rounded bg-purple-100 text-purple-700 text-[10px] font-bold">
                West
              </span>
            </div>
          </div>
              ...
              >
                <div>
                  ...
                  <div className="flex items-center justify-between gap-2 mb-3">
                    ...
                    <span
                      className={`text-[11px] font-extrabold px-3 py-1 rounded-full uppercase tracking-wider shadow-2xs ${getCorpColor(item.corp)}`}
                    >
                      {item.corp} Corp
                    </span>
                  </div>
                ...
                <div className="space-y-2 pt-4 border-t border-slate-100">
                  ...
                  <button
                    onClick={() =>
                      navigate(
                        `/complaints?constituency=${encodeURIComponent(item.constituency)}&corp=${encodeURIComponent(item.corp)}`,
                      )
                    }
          ...
          {filteredWards.length === 0 && (
            <div className="text-center py-20 bg-white rounded-3xl border border-slate-200">
              ...
              <h3 className="text-xl font-bold text-dark">
                No constituencies found matching "{mapSearch}"
              </h3>
              <p className="text-slate-500 text-sm mt-1">
                Try selecting 'All Corporations' or search for names like
                Jayanagar, Malleshwaram, Hebbal.
              </p>
            </div>
          )}
        </div>
                  ...
                  {itemsInGroup.map((item, i) => (
                    ...
                    >
                      <div className="flex items-start gap-4 relative z-10">
                        <div
                          className={`w-12 h-12 rounded-xl flex items-center justify-center shrink-0 bg-gradient-to-br from-red-600 to-amber-500 text-white shadow-inner`}
                        >
                          <item.icon size={20} className="drop-shadow-sm" />
                        </div>
                        <div className="flex-1 min-w-0">
                          <div className="text-[10px] font-black tracking-wider uppercase text-red-600 mb-1">
                            {item.role}
                          </div>
                          <h3 className="font-bold text-dark leading-tight mb-3 text-[15px] line-clamp-2">
                            {item.name}
                          </h3>

                          <a
                            href={`tel:${item.phone}`}
                            className="inline-flex items-center gap-1.5 text-sm font-bold text-slate-600 hover:text-red-600 transition-colors bg-slate-50 px-3 py-1.5 rounded-lg border border-slate-200 group-hover:border-red-500/20 group-hover:bg-red-50/50"
                          >

    ...
    >
      ...
      >
        <div className="w-1 h-full bg-white shadow-[0_0_10px_rgba(0,0,0,0.5)] flex items-center justify-center relative">
          <div
            className={`w-8 h-8 bg-white rounded-full shadow-xl flex items-center justify-center border-2 border-slate-200 text-slate-800 absolute transition-all duration-300 ${isDragging ? "scale-110 bg-primary border-primary text-white" : "group-hover/slider:scale-110"}`}
          >
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <section className="relative overflow-hidden pt-36 pb-24 flex items-center bg-white border-b-8 border-brand-orange">
        ...
        />
        <motion.div
          animate={{ scale: [1, 1.5, 1], x: [0, 60, 0] }}
        ...
        <div className="relative max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          >
            <Sparkles size={16} className="animate-pulse" /> Live Progress
            Gallery
          </motion.div>
          <motion.h1
            initial={{ opacity: 0, y: 20 }}
          ...
          >
            Real issues. Real solutions. Documented before & after by your ward
            administration for full transparency.
          </motion.p>
          ...
          >
            {[
              { label: "Activities Posted", val: beforeAfter.length },
              {
                ...
                val: beforeAfter.filter((b) => !b.type).length,
              },
              { label: "Volunteers Engaged", val: "200+" },
            ].map((s, i) => (
              <div
                key={i}
                className="bg-white border border-slate-200 shadow-sm rounded-2xl px-8 py-4 text-center"
              >
        ...
        {filtered.length === 0 ? (
          ...
          >
            ...
            <p className="text-slate-500 font-medium max-w-md mx-auto">
              As ward activities are published and complaints are resolved with
              photos, they will appear here automatically.
            </p>
          </motion.div>
        ) : (
              ...
              >
                      ...
                      <img
                        src={
                          item.images && item.images[0]
                            ? item.images[0]
                            : item.after ||
                              "https://images.unsplash.com/photo-1544027993-37dbfe43552e?auto=format&fit=crop&q=80&w=600"
                        }
                        alt={item.title}
                ...
                <div className="p-5 pt-0">
                  <div className="flex items-center justify-between mb-3">
                    <span
                      className={`inline-flex items-center px-4 py-1 rounded-full text-[10px] uppercase tracking-widest font-black text-white bg-gradient-to-r ${getCategoryGradient(item.category)} shadow-sm`}
                    >
                  ...
                  <div className="flex items-center justify-between mt-4 pt-4 border-t border-slate-100">
                    <span className="text-[11px] text-slate-500 font-bold uppercase tracking-wider flex items-center gap-1.5">
                      ...
                      {item.type === "standalone"
                        ? "Ward Activity"
                        : WARD_LABELS[item.ward] || "Bengaluru Ward"}
                    </span>
          ...
          >
            ...
            >
              ...
              <div className="md:w-80 bg-white p-8 flex flex-col overflow-y-auto">
                {/* Category badge */}
                <span
                  className={`inline-flex items-center px-4 py-1.5 rounded-full text-xs font-black uppercase tracking-widest text-white bg-gradient-to-r ${getCategoryGradient(selected.category)} mb-6 w-max shadow-md`}
                >
                ...
                <div className="bg-slate-50 rounded-2xl p-5 space-y-4 border border-slate-100 mb-6">
                  <div className="flex items-center gap-3 text-sm">
                    ...
                    <span className="text-slate-700 font-bold">
                      {selected.type === "standalone"
                        ? "Ward Activity"
                        : WARD_LABELS[selected.ward] || "Bengaluru Ward"}
                    </span>
                  </div>
                  <div className="flex items-center gap-3 text-sm">
                  ...
                  {selected.tags && (
                    <div className="flex flex-wrap gap-2 pt-3 border-t border-slate-200 mt-2">
                      {selected.tags.split(",").map((t, i) => (
                        <span
                          key={i}
                          className="bg-white text-slate-500 text-[10px] font-black uppercase tracking-widest px-3 py-1.5 rounded-md border border-slate-200 shadow-sm"
                        >
                          #{t.trim()}
                        </span>
                      ))}
